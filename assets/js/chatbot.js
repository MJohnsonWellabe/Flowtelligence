...
(function () {
  ...
  const SITE_CONTENT = `
=== Flowtelligence Website Content ===

COMPANY OVERVIEW
Flowtelligence builds intelligent insurance application technology. Core product: Flowtelligence Orchestrator (enrollment and underwriting workflow engine). Full tool suite: Flowtelligence Engage.
...
OUR TOOLS
Flowtelligence Pulse: Real-time visual view of business activity across products and geographies.
MatchIQ: Automatically pairs customers with most appropriate product. Removes guesswork for agents.
Flowtelligence Link: Integrated messaging within the enrollment workflow. Reduces follow-ups and off-platform coordination.
Flowtelligence Spark: Real-time incentive visibility embedded in enrollment. Aligns agent behavior with carrier objectives immediately.
...
Flowtelligence LeadFlow: Supports digital lead capture and routing into guided enrollment workflows.

CASE STUDIES
Time to Market: Mid-sized regional carrier. Challenge: slow product change cycles. Solution: Flowtelligence as system of engagement. Impact: more frequent enhancements, shorter launch cycles (months to weeks), increased experimentation, year-over-year growth more than doubled.

Business Growth (Wellabe): Mutual insurance organization. Challenge: growing faster without operational strain. Solution: Flowtelligence for rapid iteration. Impact: more rapid product iteration, shortened launch timelines, year-over-year sales more than doubled, sustainable execution at scale.

Quality of Business: Multi-product carrier. Challenge: inconsistent application quality in certain channels. Solution: Flowtelligence to apply guardrails consistently, guide applicants toward higher-quality submissions. Impact: higher underwriting success rates, improved consistency across channels, access to previously avoided distribution segments.
...
Bill Butters — Lead Developer: Principal architect of the Flowtelligence Orchestrator platform. Decades of carrier technology experience. Built the modern buying and underwriting platform at prior carrier that enabled rapid iteration and dramatically higher sales volume.
...
FAQ
What is Flowtelligence? A configurable buying and underwriting platform that combines modern enrollment, real-time underwriting decisioning, and data visibility in a single, carrier-designed platform.
...
`;
  ...
  // ── SYSTEM PROMPT ────────────────────────────────────────────────────────────
  const SYSTEM_PROMPT = `You are a knowledgeable, credible product specialist for Flowtelligence, a carrier-facing intelligent insurance application platform. Key products: Flowtelligence Orchestrator (core enrollment engine) and Flowtelligence Engage (full tool suite). Your audience includes life & health carrier executives, product leaders, distribution leaders, and technology teams.

Critical constraints (never violate these):
1. You may ONLY answer questions using information explicitly provided in the Flowtelligence website content passed to you in each message.
...
6. Do NOT compare Flowtelligence or its products to competitors unless the site explicitly does so.
...
Your goal: Help serious carrier stakeholders quickly understand whether Flowtelligence Orchestrator and Flowtelligence Engage are relevant to their operating model — with clarity, honesty, and credibility.`;
  ...
  widget.innerHTML = `
    <div id="me-chat-window">
      <div id="me-chat-header">
        ...
        <div class="me-info">
          <div class="me-name">Flowtelligence Assistant</div>
      ...
      <div id="me-chat-footer">Powered by Flowtelligence · AI responses may vary</div>
    </div>
  ...
  async function sendMessage() {
    ...
    try {
      ...
      hideTyping();
      addMessage('bot', "I'm having trouble connecting right now. Please try again or reach out via the Contact page.");
      console.error('Flowtelligence chatbot error:', err);
    }
